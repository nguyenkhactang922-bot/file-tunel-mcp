...
|---|---|---|---|
| V11-001 | READY | - | Bounded logical session/correlation lifecycle: TTL, cap, cleanup, never evict in-flight, pressure metrics, concurrency tests |
...
## Dependency graph`r`n`r`nV11-001 --> V11-002 -----------+`r`nV11-003 --> V11-004 -----------+--> V11-008 --> OBS-013 --> V11-009`r`nV11-005 --> V11-006 --> V11-007+`r`n`r`nOBS-013 is intentionally deferred so the live ChatGPT proof exercises the fully upgraded build rather than an obsolete pre-V1.1 binary.`r`n`r`n## Deliberately excluded tasks
...
NEXT_EXACT_ACTION: claim V11-001 now. OBS-013 runs after V11-008.
